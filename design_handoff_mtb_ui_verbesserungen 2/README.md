...
## Overview
Drei entschiedene UI-Verbesserungen aus dem Design-Review plus ein vierter Umbau:
...
4. **Mitgliederverwaltung (verwaltung.tsx):** Rollen als mehrfach anklickbare Tags — Guide, Jugend-Guide, Jugend, Verwaltung (Design „5b", inkl. kleiner API-Änderung)
...
### 4. Mitgliederverwaltung — Rollen-Tags „5b" (app/verwaltung.tsx + API)
**Problem heute:** Rolle ist ein Entweder-oder (mitglied/guide/verwaltung), Jugend nur ein Textlink, „Jugend-Guide" existiert nicht — dabei macht im Verein einer oft beides.

**Neu — UI:**
- Pro Mitgliedskarte eine umbrechende Tag-Zeile mit **vier unabhängig togglebaren Chips**: Guide · Jugend-Guide · Jugend · Verwaltung. Chip-Maße wie überall: min-height 44, padding 0 12, radius 6, border 1px #b7c2c8, Barlow Medium 15. Aktiv: #25749e gefüllt, weiße Schrift, vorangestelltes `checkmark`-Icon 14px.
- Ein Tipp togglet sofort (PATCH); nur **Verwaltung vergeben/entziehen** fragt per Dialog nach (letzte Verwaltung bleibt unentziehbar — 409 der API durchreichen).
- Destruktives („Mitglied löschen", „Einladung zurückziehen") wandert in ein „…"-Menü oben rechts in der Karte (`ellipsis-horizontal`, 20px) — die Karte bleibt ruhig.
- Unter den Tags optional „Zuletzt gesehen: …" (excerpt-Stil). Eingeladene ohne Konto: nur Badge „Eingeladen" (#8c5a16) + Hinweistext, Aktionen im „…"-Menü.
- E-Mail-Feld und Einladen-Button auf min-height 44 anheben.

**Neu — API/Datenmodell (klein halten):**
- `rolle` bleibt Einzelwert mit der Hierarchie Verwaltung ⊇ Guide (api/src/rolle.ts in der Logik unverändert). Mapping der UI-Tags: Verwaltung-Chip an ⇔ rolle='verwaltung'; Guide-Chip an ⇔ rolle='guide' (bei 'verwaltung' geerbt — Guide-Chip dann aktiv, aber als impliziert/gesperrt darstellen); beide aus ⇔ rolle='mitglied'.
- **Jugend-Guide wird ein eigenes boolean-Feld** wie `jugend`: Migration 015 `ALTER TABLE mitglied ADD COLUMN jugend_guide boolean NOT NULL DEFAULT false;`, PATCH `/verwaltung/mitglieder/:id` um `jugendGuide?` erweitern, `MitgliedZeile` in src/data/verwaltung.ts ebenso. Damit ist Guide + Jugend-Guide gleichzeitig möglich, ohne das Rollenmodell umzubauen.
- Wo Jugend-Guide Rechte braucht (z.B. Jugendtrainings anlegen), analog zu `hatGuideRechte` eine Funktion `hatJugendGuideRechte(rolle, jugendGuide)` — Verwaltung erbt auch hier.
...
- Rollen-Tags: Toggle optimistisch, bei API-Fehler zurückrollen + Toast; Verwaltung-Toggle mit Bestätigungsdialog.
- Alle Tippziele ≥ 44px.
...
- Verwaltung: pro Mitglied `rolle: 'mitglied'|'guide'|'verwaltung'`, `jugend: boolean`, `jugendGuide: boolean` (neu).
...
- Icons: Ionicons (settings-outline, options-outline, chevron-back, close-circle, images-outline, chevron-down, checkmark, ellipsis-horizontal)

## Assets
Keine neuen Assets. Bild-Platzhalter im Prototyp stehen für Beitragsbilder von der Website bzw. Nutzerfotos.

## Files
- `Aktuelles.dc.html` — der Prototyp. Abschnitt „2" = Aktuelles-Filter (2a), „3" = Einstellungen (3a Ist / 3b Ziel / 3c Alternative), „4" = Foto-Upload (4a Ist / 4b + 4c Ziel), „5" = Mitgliederverwaltung (5a Ist / 5b Ziel), „1" = frühere Filter-Varianten (nur Historie).
- `image-slot.js`, `support.js` — Laufzeit für den Prototyp, im Browser öffnen genügt.

Betroffene Repo-Dateien: app/(tabs)/news.tsx · app/einstellungen.tsx · Router-Stack-Konfiguration · app/fotos/[id].tsx · src/features/fotos/FotoRaster.tsx · src/features/fotos/warteschlange.ts (nur lesen) · app/verwaltung.tsx · src/data/verwaltung.ts · api/src/rolle.ts · api/src/migrationen/ (neu: 015) · src/theme.ts (Token-Quelle).
